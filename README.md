...
Преобразователь подключен к микроконтроллеру ATmega 328P, который отфильтровывает все, кроме значений от 0x00 до 0x7F (диапазон ASCII). Это было сделано, чтобы избавиться от мусора на экране.
...
void setup() {
  // Инициализация аппаратного UART на скорости 9600 бод
  Serial.begin(9600);

  // Инициализация софтварного UART
  softSerial.begin(9600);
}

void loop() {
  // Проверка наличия данных на аппаратном UART
  if (Serial.available()) {
    byte incomingByte = Serial.read(); // Чтение входящего байта

    // Проверка, находится ли байт в диапазоне от 0x00 до 0x7F
    if (incomingByte <= 0x7F) {
      // Отправка байта обратно по софтварному UART
      softSerial.write(incomingByte);
    }
  }

  // Проверка наличия данных на софтварном UART
  if (softSerial.available()) {
    byte incomingByte = softSerial.read(); // Чтение входящего байта

    // Проверка, находится ли байт в диапазоне от 0x00 до 0x7F
    if (incomingByte <= 0x7F) {
      // Отправка байта обратно по аппаратному UART
